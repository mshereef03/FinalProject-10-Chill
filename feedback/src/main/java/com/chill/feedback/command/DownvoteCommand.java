...
@Component
public class DownvoteCommand implements FeedbackCommand {
    private final FeedbackRepository feedbackRepository;
    @Autowired
    public DownvoteCommand(FeedbackRepository feedbackRepository) {
    ...
    public Feedback execute(UUID id, UUID userId) {
        ...
        if (!(f instanceof com.chill.feedback.models.Votable)) {
            throw new ResponseStatusException(
                    HttpStatus.BAD_REQUEST, "Cannot downvote type: " + f.getClass().getSimpleName());
        }
        ((Votable)f).downvote(userId);
