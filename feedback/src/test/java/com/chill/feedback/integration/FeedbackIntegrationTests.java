...
public class FeedbackIntegrationTests {
    ...
    private static final String BASE = "http://localhost:9091/feedbacks";
    ...
    @Test @Order(13)
    void leastReviewsForVendorIntegration() {
        ...
        // fetch top‐reviews
        ResponseEntity<List> resp = rt.getForEntity(
                BASE + "/reviews/vendor/" + vendorId + "/least",
                List.class
        );
        assertEquals(200, resp.getStatusCodeValue());
        ...
        assertEquals(5, ((Number)reviews.get(1).get("rating")).intValue());
        assertEquals("Excellent!", reviews.get(1).get("comment"));
        assertEquals(2, ((Number)reviews.get(0).get("rating")).intValue());
        assertEquals("Okay service", reviews.get(0).get("comment"));
    }
    ...
    @Test @Order(21)
    void topReviewsByUserIntegration() {
        String userId = UUID.randomUUID().toString();
        // low‐rated
        Map<String,Object> low = new HashMap<>();
        low.put("type",     "review");
        low.put("userId",   userId);
        low.put("vendorId", UUID.randomUUID().toString());
        low.put("orderId",  UUID.randomUUID().toString());
        low.put("comment",  "Not great");
        low.put("rating",   1);
        rt.postForEntity(BASE, new HttpEntity<>(low, jsonHeaders()), Map.class);
        ...
        Map<String,Object> high = new HashMap<>();
        high.put("type",     "review");
        high.put("userId",   userId);
        high.put("vendorId", UUID.randomUUID().toString());
        high.put("orderId",  UUID.randomUUID().toString());
        high.put("comment",  "Fantastic!");
        high.put("rating",   5);
        rt.postForEntity(BASE, new HttpEntity<>(high, jsonHeaders()), Map.class);


        ResponseEntity<List> resp = rt.getForEntity(
                BASE + "/reviews/user/" + userId + "/top",
                List.class
        );
        assertEquals(200, resp.getStatusCodeValue());
        @SuppressWarnings("unchecked")
        ...
        assertEquals(5, ((Number)list.get(0).get("rating")).intValue());
        assertEquals("Fantastic!", list.get(0).get("comment"));
    }


    @Test @Order(22)
    void leastReviewsByUserIntegration() {
        String userId = UUID.randomUUID().toString();
        // mid‐rated
        Map<String,Object> mid = new HashMap<>();
        mid.put("type",     "review");
        mid.put("userId",   userId);
        mid.put("vendorId", UUID.randomUUID().toString());
        mid.put("orderId",  UUID.randomUUID().toString());
        mid.put("comment",  "Okay");
        mid.put("rating",   3);
        rt.postForEntity(BASE, new HttpEntity<>(mid, jsonHeaders()), Map.class);
        ...
        low.put("vendorId", UUID.randomUUID().toString());
        low.put("orderId",  UUID.randomUUID().toString());
        low.put("comment",  "Terrible");
        low.put("rating",   1);
        rt.postForEntity(BASE, new HttpEntity<>(low, jsonHeaders()), Map.class);

        // call least‐by‐user
        ResponseEntity<List> resp = rt.getForEntity(
                BASE + "/reviews/user/" + userId + "/least",
                List.class
        );
        assertEquals(200, resp.getStatusCodeValue());
        @SuppressWarnings("unchecked")
        List<Map<String,Object>> list = resp.getBody();
        assertEquals(2, list.size());
        assertEquals(1, ((Number)list.get(0).get("rating")).intValue());
        assertEquals("Terrible", list.get(0).get("comment"));
    }


    @Test @Order(23)
    void rootThreadsByUserIntegration() {
        String userId = UUID.randomUUID().toString();
        // create a root thread
        Map<String,Object> root = new HashMap<>();
        root.put("type",     "thread");
        root.put("userId",   userId);
        root.put("vendorId", UUID.randomUUID().toString());
        root.put("orderId",  UUID.randomUUID().toString());
        root.put("comment",  "Root question");
        root.put("parentId", null);
        ResponseEntity<Map> rootResp = rt.postForEntity(BASE,
                new HttpEntity<>(root, jsonHeaders()), Map.class);
        String rootId = rootResp.getBody().get("id").toString();

        // create a reply (sub‐thread)
        Map<String,Object> reply = new HashMap<>(root);
        reply.put("parentId", rootId);
        rt.postForEntity(BASE + "/" + rootId + "/reply",
                new HttpEntity<>(reply, jsonHeaders()), Map.class);

        // call root‐threads‐by‐user
        ResponseEntity<List> resp = rt.getForEntity(
                BASE + "/threads/root/user/" + userId,
                List.class
        );
        assertEquals(200, resp.getStatusCodeValue());
        @SuppressWarnings("unchecked")
        List<Map<String,Object>> list = resp.getBody();
        // only the root thread should appear
        assertEquals(1, list.size());
        assertEquals(rootId, list.get(0).get("id").toString());
    }


    @Test @Order(24)
    void subThreadsByUserIntegration() {
        String userId = UUID.randomUUID().toString();
        // create root
        Map<String, Object> root = new HashMap<>();
        root.put("type", "thread");
        ...
        for (int i = 0; i < 2; i++) {
            Map<String, Object> sub = new HashMap<>(root);
            sub.put("parentId", rootId);
            sub.put("comment", "Reply " + i);
            rt.postForEntity(BASE + "/" + rootId + "/reply",
                    new HttpEntity<>(sub, jsonHeaders()), Map.class);
        }

        // call sub‐threads‐by‐user
        ResponseEntity<List> resp = rt.getForEntity(
                BASE + "/threads/sub/user/" + userId,
                List.class
        );
        assertEquals(200, resp.getStatusCodeValue());
        @SuppressWarnings("unchecked")
        List<Map<String, Object>> list = resp.getBody();
        // exactly two sub‐threads
        assertEquals(2, list.size());
        for (Map<String, Object> t : list) {
            assertEquals(rootId, t.get("parentId").toString());
        }
    }
